...
bool Isempty(string input) // shows eeror for empty input.
{
    if (input.empty())
    {
        cout << "Error!\n"
...
Item *items = new Item[10]{
    {"pear", 10, 7}, {"greengage", 8.5, 5}, {"apple", 2.5, 8}, {"kiwi", 9, 11}, {"apricot", 6, 4}, {"grapefruit", 11.5, 3}, {"pineapple", 10.5, 12}, {"tomato", 4.5, 22}, {"banana", 13.4, 8}, {"strawberry", 15, 3}};
...
void help(string prefix)
{
    ...
    else if (prefix == "")
    {
        ...
        cout << "if you enter the name of an item , like : help show\n " << "the system will give you details on how tp use it.\n";
        cout << "To use it , enter the command name.\n";
    }
    else
    {
...
void add(string ITEM, string PRICE)
{
    ...
    if (PRICE == "")
    {
        cout << "The price cannot be empty.\n";
        return;
    }
    ...
    items[itemCount - 1] = {ITEM, prices, 1};
    cout << "The new Item has been added to the stock.\n";
    boundary();
}
...
void Remove(string secondWord)
{
    ...
    if (ans != nullptr)
    {
        ...
        {
            {
                Item removedItem = *ans;
                resizeArray(itemCount - 1, itemCount, ans);
                ProductSaveToFile();

                cout << "There is only 1 kg of item " << ans->name << " in stock, so we will remove it completely.\n";
        ...
        {
            ans->count -= 1;
            cout << "Are you sure you want to remove 1 kg of " << ans->name << " ?\n"
                 << "Enter 1 if you want to restore it.\n";
            getline(cin, restore);
            if (restore == "1")
            {
            ...
            {
                cout << "You removed 1 kg of " << ans->name << " .\n";
            }
        }
    }
    boundary();
...
void price(string secondWord, string thirdWord)
{
    ...
    if (secondWord == "" || thirdWord == "")
    {
        cout << "The item's name and pirce cannot be empty.\n";
        return;
    }
    if(!ErrorsOfNumbers(thirdWord))
    {
        return;
    }
    Item *ans = find(secondWord);
